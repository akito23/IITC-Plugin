...
// @category     Layer
// @version      1.182
// @description  Place markers on the map for your candidates in Recon.
...
function wrapper(pluginInfo) {
    ...
    const STORAGE_KEY = "recon_planner_settings";
    const LEGACY_STORAGE_KEYS = ["wayfarer_planner_settings"];

    let settings = defaultSettings;

    function escapeHtml(value) {
        const strValue = value == null ? "" : String(value);
        return strValue.replace(/[&<>"']/g, (ch) => {
            const entities = {
                "&": "&amp;",
                "<": "&lt;",
                ">": "&gt;",
    ...
    function loadSettings() {
        let tmp = localStorage[STORAGE_KEY];
        let loadedFromLegacyKey = false;
        if (!tmp) {
            for (let i = 0; i < LEGACY_STORAGE_KEYS.length; i++) {
                const legacyValue = localStorage[LEGACY_STORAGE_KEYS[i]];
                if (legacyValue) {
                    tmp = legacyValue;
        ...
        $.ajax({
            ...
            success: function (data, status, header) {
                try {
                    ...
                    console.log("Recon Planner. Exception parsing response: ", e); // eslint-disable-line no-console
                    alert("Recon Planner. Exception parsing response.");
                    return;
                }
            ...
            error: function (x, y, z) {
                console.log("Recon Planner. Error message: ", x, y, z); // eslint-disable-line no-console
                alert(
                    "Recon Planner. Failed to retrieve data from the scriptURL.\r\nVerify that you're using the right URL and that you don't use any extension that blocks access to google.",
                );
            },
        });
    }
    ...
    function processCustomMarkers() {
        markercollection.forEach((marker) => {
            marker.status = normalizeMarkerStatus(marker.status);
        });
        ...
        for (const marker of newMarkers) {
            ...
            mapLayers[markerStatus] = {
                title: markerStatus.charAt(0).toUpperCase() + markerStatus.slice(1),
            };
        }
    ...
    function initializeLayers() {
        Object.values(mapLayers).forEach((data) => {
            if (!data.initialized) {
                ...
                data.layer = layer;
                window.addLayerGroup("Recon - " + data.title, layer, true);
                layer.on("click", (e) => {
                    markerClicked(e);
                });
    ...
    function drawMarker(candidate) {
        ...
        candidate.status = normalizeMarkerStatus(candidate.status);
        if (!mapLayers[candidate.status]) {
            mapLayers[candidate.status] = {
                title:
        ...
        if (!settings.markers[candidate.status]) {
            settings.markers[candidate.status] = {
                submitRadius: true,
                interactRadius: true,
            };
        }
        mapLayers[candidate.status].color =
            settings.markers[candidate.status].color ||
            mapLayers[candidate.status].color ||
            "#27AE60";
        if (!mapLayers[candidate.status].initialized) {
            initializeLayers();
        }
    ...
    function addVotingProximity(candidate) {
        if (settings.showVotingProximity && candidate.status === "voting") {
            ...
            for (let i = 0; i < surrounding.length; i++) {
                ...
                if (plottedcells[cellId].candidateIds.indexOf(candidate.id) === -1) {
    ...
    function drawInputPopop(latlng, markerData) {
        ...
        if (markerData !== undefined) {
            id = markerData.id == null ? "" : String(markerData.id);
            title = markerData.title == null ? "" : String(markerData.title);
            description =
                markerData.description == null
                    ? ""
                    : String(markerData.description);
            submitteddate =
                markerData.submitteddate == null
                    ? ""
                    : String(markerData.submitteddate);
            nickname =
                markerData.nickname == null ? "" : String(markerData.nickname);
            status =
                markerData.status == null ? status : String(markerData.status);
            imageUrl =
                markerData.candidateimageurl == null
                    ? ""
                    : String(markerData.candidateimageurl);

            const parsedLat = Number.parseFloat(markerData.lat);
            const parsedLng = Number.parseFloat(markerData.lng);
            lat = Number.isFinite(parsedLat)
                ? parsedLat.toFixed(6)
                : latlng.lat.toFixed(6);
            lng = Number.isFinite(parsedLng)
                ? parsedLng.toFixed(6)
        ...
        if (!Object.prototype.hasOwnProperty.call(mapLayers, status)) {
            status = "potential";
        }
        ...
        const options = Object.keys(mapLayers)
            .map((markerId) => {
                const selected = markerId === status ? ' selected="selected"' : "";
                const optionTitle = mapLayers[markerId].title || markerId;
                return `<option value="${escapeHtml(markerId)}"${selected}>${escapeHtml(optionTitle)}</option>`;
            })
            .join("");
        let coordinates = `<input name="lat" type="hidden" value="${escapeHtml(lat)}">
            <input name="lng" type="hidden" value="${escapeHtml(lng)}">`;
        if (settings.enableCoordinatesEdit) {
            coordinates = `<label>Latitude
                <input name="lat" type="text" autocomplete="off" value="${escapeHtml(lat)}">
                </label>
                <label>Longitude
                <input name="lng" type="text" autocomplete="off" value="${escapeHtml(lng)}">
                </label>`;
        }

        let image = "";
        const editableImageUrl = imageUrl;
        let previewImageUrl = sanitizeHttpUrl(imageUrl);
        let largeImageUrl = previewImageUrl;
        if (previewImageUrl && previewImageUrl.includes("googleusercontent")) {
            largeImageUrl = largeImageUrl.replace(/(=.*)?$/, "=s0");
            previewImageUrl = previewImageUrl.replace(/(=.*)?$/, "=s200");
        }
        if (previewImageUrl && settings.enableImagePreview) {
            image = `<a href="${escapeHtml(largeImageUrl)}" target="_blank" rel="noopener noreferrer" class="imagePreviewContainer"><img class="imagePreview loading" src="${escapeHtml(previewImageUrl)}"></a>`;
        }

        const playerNickname =
            window.PLAYER && window.PLAYER.nickname ? window.PLAYER.nickname : "";

        let formContent = `<div class="recon-planner-popup"><form id="submit-to-recon">
            <label>Status
            <select name="status">${options}</select>
            </label>
            <label>Title
            <input name="title" type="text" autocomplete="off" placeholder="Title (required)" required value="${escapeHtml(title)}">
            </label>
            <label>Description
            <input name="description" type="text" autocomplete="off" placeholder="Description" value="${escapeHtml(description)}">
            </label>
            ${image}
            <div class='recon-expander' title='Click to expand additional fields'>»</div>
            <div class='recon-extraData'>
            ${coordinates}
            <label>Submitter
            <input name="submitter" type="text" value="${escapeHtml(nickname)}" disabled>
            </label>
            <label>Submitted date
            <input name="submitteddate" type="text" autocomplete="off" placeholder="dd-mm-jjjj" value="${escapeHtml(submitteddate)}">
            </label>
            <label>Image URL
            <input name="candidateimageurl" type="text" autocomplete="off" placeholder="http://?.googleusercontent.com/***" value="${escapeHtml(editableImageUrl)}">
            </label>
            </div>
            <input name="id" type="hidden" value="${escapeHtml(id)}">
            <input name="nickname" type="hidden" value="${escapeHtml(playerNickname)}">
            <button type="submit" id='recon-submit'>Send</button>
            </form>`;
        ...
        if (largeImageUrl && !settings.enableImagePreview) {
            formContent +=
                ' <a href="' +
                escapeHtml(largeImageUrl) +
                '" style="padding:4px; float:right;" target="_blank" rel="noopener noreferrer">Image</a>';
        }
        const align =
            id !== ""
                ? "float: right"
                : "box-sizing: border-box; text-align: right; display: inline-block; width: 100%";
        formContent += ` <a href="https://www.google.com/maps?layer=c&cbll=${encodeURIComponent(lat)},${encodeURIComponent(lng)}" style="padding:4px; ${align};" target="_blank" rel="noopener noreferrer">Street View</a>`;

        formpopup.setContent(formContent + "</div>");
        formpopup.openOn(map);

        const contentNode = formpopup._contentNode;
        const deleteLink = contentNode.querySelector("#deletePortalCandidate");
        if (deleteLink != null) {
            deleteLink.addEventListener("click", (e) =>
                confirmDeleteCandidate(e, id),
            );
        }
        const expander = contentNode.querySelector(".recon-expander");
        if (expander) {
            expander.addEventListener("click", function () {
                expander.parentNode.classList.toggle("recon__expanded");
            });
        }
        const previewImageElement = contentNode.querySelector(".loading");
    ...
    function confirmDeleteCandidate(e, id) {
        ...
        $.ajax({
            ...
            error: function (x, y, z) {
                console.log("Recon Planner. Error message: ", x, y, z); // eslint-disable-line no-console
                alert("Recon Planner. Failed to send data to the scriptURL");
            },
        });
    }
    ...
    function editMarkerColors() {
            ...
            if (!hideCheckboxes) {
                ...
                if (settings.showRadius) {
                    html += `<div class="options-radius-row">
                            <input type='checkbox' id='${markerId}.submitRadius' ${submitRadius ? "checked" : ""
                        } value=true>
                ...
                if (settings.showInteractionRadius) {
                    html += `<div class="options-radius-row">
                            <input type='checkbox' id='${markerId}.interactRadius' ${interactRadius ? "checked" : ""
                        } value=true>
        ...
        div.addEventListener("change", (event) => {
            ...
            if (event.target.type === "checkbox") {
                ...
                document.getElementById(`marker.${markerId}.colorPicker`).value = value;
                const svg = document.getElementById(`marker.${markerId}.svg`);
                svg
                    .querySelectorAll("path, circle")
                    .forEach((path) => (path.style.fill = value));
    ...
    function showDialog() {
        ...
        const html = `<p><label for="txtScriptUrl">Url for the script</label><br><input type="url" id="txtScriptUrl" spellcheck="false" placeholder="https://script.google.com/macros/***/exec"></p>
             <p><a class='recon-refresh'>Update candidate data</a></p>
        ...
        const linkRefresh = div.querySelector(".recon-refresh");
        ...
        const chkShowVotingProximity = div.querySelector("#chkShowVotingProximity");
        ...
        const plannerEditMapFeatures = div.querySelector("#plannerEditMapFeatures");
    ...
    const setup = function () {
        ...
        $("<style>")
            ...
            #submit-to-recon input,
            #submit-to-recon select {
                width: 100%;
            }
            #submit-to-recon input {
                color: #CCC;
            }
            #submit-to-recon label {
                margin-top: 5px;
                display: block;
                color: #fff;
            }
            ...
            .recon__expanded .recon-expander {
                transform: rotate(270deg) translate(1px, -3px);
            }

            .recon__expanded .recon-extraData {
                max-height: none;
                margin-top: 0em;
            }
            ...
            #submit-to-recon .imagePreviewContainer{
                display:block;
                margin-top:5px;
                text-align:center;
            }
            #submit-to-recon .imagePreview{
                max-width:100%;
                max-height:150px;
            }
            .options-row {
        ...
        $("body").on("submit", "#submit-to-recon", function (e) {
            e.preventDefault();
            map.closePopup();
            $.ajax({
                url: settings.scriptURL,
                ...
                error: function (x, y, z) {
                    console.log("Recon Planner. Error message: ", x, y, z); // eslint-disable-line no-console
                    alert(
                        "Recon Planner. Failed to send data to the scriptURL.\r\nVerify that you're using the right URL and that you don't use any extension that blocks access to google.",
                    );
                },
            });
        });
        ...
        toolboxLink.textContent = "Recon";
        toolboxLink.title = "Settings for Recon Planner";
